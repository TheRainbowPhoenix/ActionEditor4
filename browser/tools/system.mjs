...
function printUsage() {
    console.error('Usage: node system.mjs <dump|build> <input> [output] [--out <file>]');
    console.error('Examples:');
    console.error('  node system.mjs dump System.dat --out System.json');
    console.error('  node system.mjs build System.json System.dat');
}
...
async function dumpSystem(inputPath, outputPath) {
    let buffer;
    try {
...
async function buildSystem(inputPath, outputPath) {
    let payload;
    try {
    ...
    let arrayBuffer;
    try {
        arrayBuffer = serializeSystem(payload);
    } catch (error) {
...
async function main(argv) {
    ...
    if (command === 'dump') {
        ...
        try {
            await dumpSystem(inputPath, outputPath);
        } catch (error) {
    ...
    try {
        await buildSystem(inputPath, outputPath);
    } catch (error) {
        console.error(error.message);
        process.exitCode = 1;
    }
