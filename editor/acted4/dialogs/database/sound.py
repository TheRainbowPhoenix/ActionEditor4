...
class SoundDialog(SideListTabDialog):
    def __init__(self, project: ProjectData, parent=None):
        ...
        if self.project.sound:
            self.init_side_list(
                self.ui.soundList,
                self.ui.soundLNewButton,
                self.ui.soundLClearButton,
                self.ui.soundLCountButton,
                self.project.sound.data.elements,
                SoundElement
            )
    ...
    def _connect_signals(self):
        ...
        # Element editing
        self.ui.soundList.currentRowChanged.connect(self._on_selected_element_changed)
    ...
    def _update_element_ui(self):
        ...
        current_row = self.ui.soundList.currentRow()
        if current_row < 0 or not self.project.sound:
            return
            
        element = self.project.sound.data.elements[self.offset(current_row)]
        self._updating = True
    ...
    def _on_name_changed(self, text):
        ...
        element = self.project.sound.data.elements[self.offset(current_row)]
        element.name = text
        self.update_list_item(self.ui.soundList, current_row, element)
        self.mark_dirty()
        
    def _update_name_from_path(self, current_row: int, element: SoundElement):
    ...
    def _on_browse_path(self):
        ...
        sounds_dir = os.path.join(self.project.path, "sound")
    ...
    def apply_changes(self) -> bool:
        ...
        return self.project.sound.save()